					...
					if(k == 38480){
						...
						if(CRC == dtmp[71]){
							...
							if(dtmp[0] == 0x42 && dtmp[1] == 0x4d){
								if(dtmp[70] == ov7725){
									ACK = OK_ACK;
									if(http_ov7725_queue != 0) xQueueSend(http_ov7725_queue, &ptr_dtmp, 1/portTICK_RATE_MS);
								}else {
									ACK = NAK;
									//ESP_LOGI(TAG, "Bad camera model. Need ov7725");
								}
								k = 0;
						...
						if(CRC == dtmp[71]){
							...
							if(dtmp[0] == 0x42 && dtmp[1] == 0x4d){
								if(dtmp[70] == mlx90640){
									ACK = OK_ACK;
									if(http_mlx90640_queue != 0) xQueueSend(http_mlx90640_queue, &ptr_dtmp, 1/portTICK_RATE_MS);
								}else {
									ACK = NAK;
									//ESP_LOGI(TAG, "Bad camera model. Need mlx90640");
								}
								k = 0;
								//bzero(dtmp, RD_BUF_SIZE);
								//free(dtmp);
